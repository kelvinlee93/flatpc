...
Class Tintuc_model extends CI_Model{
	...
	function get_tintuc(){		
		$query = $this->db->query("SELECT T.ID, T.TIEUDE, T.LOAITIN, T.MOTA, T.NOIDUNG, T.NGAYDANG, T.HINH, T.TACGIA, N.TENDANGNHAP, N.HODEM, N.TENNGUOIDUNG, L.TENLOAI FROM TINTUC T,NGUOIDUNG N, LOAITINTUC L WHERE T.TACGIA = N.ID AND T.LOAITIN = L.ID ORDER BY T.NGAYDANG DESC");
